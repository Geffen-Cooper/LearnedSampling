'''
Code taken from https://github.com/teco-kit/ISWC22-HAR/blob/main/models/TinyHAR.py
'''
...
class TinyHAR_Model(nn.Module):
    def __init__(
        ...
        for i in range(nb_conv_layers):
            ...
            if i%2 == 1:
                layers_conv.append(nn.Sequential(
                    nn.Conv2d(in_channel, out_channel, (filter_size, 1),padding=(1,0)),
                    nn.ReLU(inplace=True),#))#,
...
if __name__ == "__main__":
    ...
    # create HAR model
    model = TinyHAR_Model(input_shape=(1,1,args['window_size'],num_channels),number_class=len(args['activities']),**config,t_context=(len(args['body_parts']),args['window_size'])).cuda()
    model.eval()
    with torch.no_grad():
